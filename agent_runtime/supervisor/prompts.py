...
CODE_PEER_PROMPT = (
    ...
    "You may also agent_kb_search for more. "
      ...
      "Data can only reach the sandbox as an input_files entry, which means it is already "
      "attached to the turn or a tool you have returns it AS A FILE. web_fetch cannot supply it "
      "— it returns a page's on-topic passages, not the bytes of a dataset. So if the data is "
      "not attached and no tool returns it, SAY SO and ask for the file rather than writing a "
      "fetch that cannot run: a loader that always fails is worse than a clear request. When "
      "the data IS available, keep the reused analysis verbatim and repoint only its fetch at "
      "the staged file. "
